...
Two algorithms in Java for building Steiner trees:  one that accepts a list of unconnected points in the Euclidean plane and efficiently connects them with a tree, and one that accepts an undirected graph with weighted edges and a list of some of the graph's vertices, whereupon it efficiently connects those particular vertices with a tree-shaped subgraph of the input graph.
...
1) SteinerSubgraphOptions.maximalJumpDepthInMultipleVertexJiggling == 0
2) SteinerSubgraphOptions.maximalMultipleVertexJumpCountPerPatch == Integer.MAX_VALUE
3) SteinerSubgraphOptions.random != null
